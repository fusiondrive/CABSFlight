...
public final class CABSMockEngine {
    ...
    /// Coordinates are illustrative — they trace a believable loop around
    /// the west side of campus (Lincoln Tower → Carmack → RPAC → Drinko).
    /// Replace with real GTFS shapes when wiring the live feed.
    ...
    public init() {
        let seedBuses: [MockBus] = [
            MockBus(
                id: "bus-cabs-001",
                routeName: "CLNS",
                colorHex: "#C8102E", // Buckeye scarlet
        ...
        // Seed each bus at its starting path index.
    ...
    /// Begin the 1-second simulation tick. Safe to call multiple times.
    public func start() {
        guard !isRunning else { return }
        isRunning = true
        // RunLoop.main keeps the timer alive while the engine is observed by
        // SwiftUI; we drop the strong cycle by capturing `self` weakly.
    ...
    /// First prediction matching the given stop+route, if any. Handy for the
    /// Live Activity which is scoped to a single (stop, route) pair.
    ...
    /// Convenience for ActivityKit: returns the Date at which the next bus
    /// is expected, given a (stop, route) pair. Falls back to +5 min.
    ...
    private func tick() {
        ...
        // 1. Decrement each prediction; recycle on hit-zero.
        predictions = predictions.map { p in
            var next = p
            if next.timeToArrivalInSeconds > 0 {
                next.timeToArrivalInSeconds -= 1
            } else {
                // Bus has "arrived"; the next one is 4–9 minutes out.
                next.timeToArrivalInSeconds = Int.random(in: 240...540)
            }
            return next
        }

        // 2. Advance each bus along the West Campus Loop, easing between nodes.
        buses = buses.map { bus in
            ...
            // Interpolate 10% of the way from current → next node per tick.
            ...
            // If we're within ~10 meters of the next node, advance the index.
...
public extension MockBus {
    /// SwiftUI Color derived from `colorHex`. Falls back to gray on a malformed
    /// hex string so the UI never crashes on a typo.
    var uiColor: Color {
...
public extension Color {
    /// Lenient hex initializer. Accepts "#RRGGBB", "RRGGBB", "#RRGGBBAA".
    init?(hex: String) {
...
public extension CABSMockEngine {
    ...
    /// and Live Activity snapshot rendering.
    static func previewEngine() -> CABSMockEngine {
        let e = CABSMockEngine()
